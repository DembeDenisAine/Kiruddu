<?php
    class Smsgateway
    {
        ...
        {                       
            $url = "https://rest.nexmo.com/sms/json?api_key=".urlencode($config['from'])."&api_secret=".urlencode($config['password'])."&to=".urlencode($config['to'])."&from=".urlencode($config['from'])."&text=".urlencode($config['message'])."";                       
            try {
        ...
        {
           $url = "https://platform.clickatell.com/messages/http/send?apiKey=".urlencode($config['from'])."==&to=".urlencode($config['to'])."&content=".urlencode($config['message'])."";
        ...
        {                       
            $url = "http://...?api_key=".urlencode($config['from'])."&type=text&contacts=".urlencode($config['to'])."&senderid=".urlencode($config['username'])."&msg=".urlencode($config['message'])."";                       
            try {
                return @file_get_contents($url);
            }
            catch (Exception $e) {
