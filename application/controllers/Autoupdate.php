...
//Get Update file
define('MIN_VERSION','1.0');
//Get Update file
define('MAX_VERSION','1.9');
//Get Update file
define('UPDATE_URL','');
...
class Autoupdate extends CI_Controller
{
    ...
    public function index(){ 
        ...
        if (!$this->session->userdata('isLogIn') && !$this->session->userdata('isAdmin'))
            redirect(base_url());  
        $data = array();     
        $data['module'] = display('version');
        $data['title'] = display('auto_update');
        ...
        $data['content'] = $this->load->view('autoupdate/autoupdate', $data, true);
